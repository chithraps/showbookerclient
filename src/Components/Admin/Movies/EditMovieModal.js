...
function EditMovieModal({ movie, onClose, onUpdate }) {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      try {
        ...
        const response = await axios.get(`${baseUrl}/admin/viewGenres`,{          
            headers: {
  ...
  const validateForm = () => {
    ...
    if (!updatedMovie.release_date) newErrors.release_date = 'Release date is required';
    if (!updatedMovie.duration) newErrors.duration = 'Duration is required';
    if (!updatedMovie.description) newErrors.description = 'Description is required';
    ...
    if (!updatedMovie.trailer_url) newErrors.trailer_url = 'Trailer URL is required';
    setErrors(newErrors);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const baseUrl = process.env.REACT_APP_BASE_URL;
      const response = await axios.put(`${baseUrl}/admin/updateMovie/${updatedMovie._id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      ...
      Swal('Error', error.response?.data?.message || 'An error occurred while updating the movie', 'error');
    }
  };
        ...
        <form onSubmit={handleFormSubmit}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              accept="image/*"
              name="poster"
              onChange={(e) => setUpdatedMovie((prev) => ({ ...prev, poster: e.target.files[0] }))}
              className="w-full p-2 border border-gray-300 rounded mt-1"
            />
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 mb-2">
              <input
                ...
                onChange={(e) => setCastMember({ ...castMember, actor_name: e.target.value })}
              ...
              <input
                ...
                onChange={(e) => setCastMember({ ...castMember, character_name: e.target.value })}
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 mb-2">
              <input
                ...
                onChange={(e) => setCrewMember({ ...crewMember, crew_member: e.target.value })}

...
function EditShowTimingsModal({ isOpen, onClose, onSave, timing }) {
  ...
  const [availableTimings, setAvailableTimings] = useState([
    '10:30 AM', '11:00 AM', '2:30 PM', '3:00 PM', '6:00 PM', '6:30 PM', '9:00 PM', '9:30 PM'
  ]);
  ...
  useEffect(() => {
    if (timing) {
      ...
      const allTimings = Array.from(new Set([...availableTimings, ...timing.timings]));
  ...
  const validateTiming = (newTiming) => {
    ...
    return timings.every(timing => Math.abs(newTime - parseTime(timing)) >= 180);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(`${baseUrl}/tmAdmin/updateShowTiming/${timing._id}`, {
